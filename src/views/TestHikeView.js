...
export default function NotBeingCalled() {
  const [hikeImages, setHikeImages] = useState([]);
  //   const { singleHike } = this.props.location.state;
  //   const { user, userTrips } = this.props;
  const location = useLocation();
  const { singleHike } = location.state;
  const { title } = singleHike;
  const newTitle = title.replace(/Hike /g, '');

  useEffect(() => {
    if (!hikeImages.length) {
      const result = [];
      singleHike.images.map((img) => result.push(img));
      result.shift();
      setHikeImages(result);
    }
  }, [singleHike.images, hikeImages]);
